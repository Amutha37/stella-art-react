...
import { ServiceOption } from "../../../components/Navbar/dropDownItems";
import "./contact.css";
...
const Contact = () => {
  ...
  fetch(
    'https://v1.nocodeapi.com/amutha/google_sheets/sDBpXXCxMheMRxIY?tabId=Service',
    requestCountOptions
  )
  .then((response) => response.text())
  .catch((error) => console.log("error", error));
...
// };
const handleService = (e) => {
const copyServiceOption = { ...countService };
copyServiceOption[e.label] = 1;
setService(copyServiceOption);
};
const reset = () => {
setFormData({});
};
